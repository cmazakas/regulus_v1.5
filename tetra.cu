...
/*
__global__
void compare_distance(const int *td, 
                      const int *pd, 
                      const int *ta, 
                      const int *pa, 
                      const int n, 
                      int *tpi, const point *points)
{
    // n = number of points
    ...
    for (int i = tid; i < n; i += blockDim.x * gridDim.x)
    {
        ...
        if (pd[i] == td[ta[i]]) 
        {
            atomicMin(tpi + ta[i], i); // write min index to tetra
            printf("%.00f\n", __int_as_float(pd[i]));
            printf("i vs pa[i] : %d vs %d\n", i, pa[i]);
            point p = *(points + pa[*(tpi + ta[i])]);
            printf("%.00f, %.00f, %.00f\n", p.x, p.y, p.z);

            //printf("%d\n", pa[i]);
            printf("%d\n", *(tpi + ta[i]));

            // tpi[ta[i]] = index of point
        }
    }
}
...
__global__
void resolve_conflicts(const int* ht,
                       const int* ta,
                             int* pn,
                             int* tn, // nominated tetra
                       const int  nb)
{
    const int tid = threadIdx.x + blockIdx.x * blockDim.x;

    for (int i = tid; i < nb ; i += blockDim.x * gridDim.x)
    {
        if (pn[i] == 0)
            return;

        const int start = (i > 0 ? ht[i - 1] : 0);
        const int end = ht[i];

        for (int j = start; j < end; ++j)
        {
            const int old = atomicCAS(tn + ta[j], 0, 1);
...
__global__
void compare_distance(const int* pd,
                      const int* td,
                      const int* ht,
                      const int* ta,
                            int *pn,
                      const int  nb)
{
    ...
    for (int i = tid; i < nb; i += blockDim.x * gridDim.x)
    {
        ...
        // j = location in ta and pa => point = pa[j]

        for (int j = start; j < end; ++j)
        {
            if (pd[j] == td[ta[j]])
            {
                pn[i] = 1; // nominate a bucket for insertion
...
__global__
void compute_distance(const PredicateInfo  preds, // predicate info
                      const point*         points, // point set
                      const tetra*         mesh, // mesh
                      const int*           ht, // look-up table
                            int*           td, // min dist per tetra
                            int*           pd, // dist per point
                      const int*           ta, // bucket contents
                      const int            nb) // number of buckets
{
    ...
    for (int i = tid; i < nb; i += blockDim.x * gridDim.x)
    {
        ...
        for (int j = start; j < end; ++j)
        {
            ...
            // Calculate insphere value
            const float s = insphere(preds, a.p, b.p, c.p, d.p, e.p);
            const int   ss = __float_as_int(s); 

            pd[j] = ss;

            if (ss > td[ta[j]])
                atomicMax(td + ta[j], ss);          
        }

        //printf("There are %d items in bucket %d\n", num_items, i);
    }
} 
*/
